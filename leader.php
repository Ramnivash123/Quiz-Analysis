...
if ($showResults) {
    ...
    foreach ($students as $student) {
        ...
        foreach ($result as $row) {
            if ($row['student_name'] === $student) {
                $quizTime = $row['completion_time'] / 60; // Convert to minutes
                break;
            }
        }

        // Find enhanced quiz time
        foreach ($result2 as $row) {
            if ($row['student_name'] === $student) {
                $enhancedTime = $row['completion_time'] / 60; // Convert to minutes
                break;
            }
        }

        $quizTimes[] = $quizTime;
        $enhancedTimes[] = $enhancedTime;
    }
}
?>
    ...
    <div class="flex-grow flex flex-col items-center px-4 py-6">
        <div class="w-full max-w-4xl bg-white shadow-lg rounded-lg p-6">
            ...
            <?php if ($showResults) { ?>
                ...
                <!-- Bar Chart for Quiz vs Enhanced Quiz Completion Times -->
                <h3 class="text-xl font-bold text-green-700 text-center mt-6">Quiz vs Enhanced Quiz Completion Times</h3>
                <div class="mt-6">
                ...
                <script>
                    // Prepare data for the chart
                    const studentNames = <?php echo json_encode($students); ?>;
                    const quizTimes = <?php echo json_encode($quizTimes); ?>;
                    const enhancedTimes = <?php echo json_encode($enhancedTimes); ?>;
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [
                                {
                                    label: 'Quiz Completion Time (minutes)',
                                    data: quizTimes,
                                    backgroundColor: 'rgba(75, 192, 192, 0.6)', // Light blue for quiz
                                    borderColor: 'rgba(75, 192, 192, 1)',
                                    borderWidth: 1
                                },
                                {
                                    label: 'Enhanced Quiz Completion Time (minutes)',
                                    data: enhancedTimes,
                                    backgroundColor: 'rgba(255, 99, 132, 0.6)', // Light red for enhanced quiz
                                ...
                                tooltip: {
                                    callbacks: {
                                        label: function(context) {
                                            ...
                                            return `${studentName}: ${time} minutes`;
                                        }
                                    }
                                }
                            }
